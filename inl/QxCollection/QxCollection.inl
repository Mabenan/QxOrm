...
template <typename Key, typename Value>
QxCollection<Key, Value>::QxCollection(const QxCollection<Key, Value> &other)
    : IxCollection(), m_batch(false) {
...
template <typename Key, typename Value>
bool QxCollection<Key, Value>::operator==(
    const QxCollection<Key, Value> &other) const {
...
template <typename Key, typename Value>
void QxCollection<Key, Value>::cloneCollection(
    QxCollection<Key, Value> *pClone, const QxCollection<Key, Value> &pRef) {
  if (!pClone) {
    return;
  }
  if (pClone == (&pRef)) {
...
template <typename Key, typename Value>
bool QxCollection<Key, Value>::isSameCollection(
    const QxCollection<Key, Value> *p1,
...
template <typename Key, typename Value>
void QxCollection<Key, Value>::updateHashPosition(long from /* = 0 */,
                                                  long to /* = -1 */,
                                                  bool check /* = false */) {
  if (m_batch) {
    return;
...
template <typename Key, typename Value>
typename QxCollection<Key, Value>::const_iterator
QxCollection<Key, Value>::begin() const {
  QMutexLocker locker(&m_mutex);
  return m_list.begin();
}
...
template <typename Key, typename Value>
typename QxCollection<Key, Value>::const_iterator
QxCollection<Key, Value>::end() const {
  QMutexLocker locker(&m_mutex);
  return m_list.end();
}
...
template <typename Key, typename Value>
typename QxCollection<Key, Value>::reverse_iterator
QxCollection<Key, Value>::rbegin() {
  QMutexLocker locker(&m_mutex);
...
template <typename Key, typename Value>
typename QxCollection<Key, Value>::reverse_iterator
QxCollection<Key, Value>::rend() {
  QMutexLocker locker(&m_mutex);
  return m_list.rend();
}

template <typename Key, typename Value>
typename QxCollection<Key, Value>::const_reverse_iterator
QxCollection<Key, Value>::rbegin() const {
  QMutexLocker locker(&m_mutex);
  return m_list.rbegin();
}
...
template <typename Key, typename Value>
typename QxCollection<Key, Value>::const_reverse_iterator
QxCollection<Key, Value>::rend() const {
  QMutexLocker locker(&m_mutex);
  return m_list.rend();
}
...
template <typename Key, typename Value> void QxCollection<Key, Value>::clear() {
  QMutexLocker locker(&m_mutex);
  m_hash.clear();
  m_list.clear();
}
...
template <typename Key, typename Value>
bool QxCollection<Key, Value>::insert(long index, const Key &key,
                                      const Value &value) {
  qAssert(!exist(key));
  if (index < 0) {
    index = 0;
...
template <typename Key, typename Value>
bool QxCollection<Key, Value>::insert(long index,
...
template <typename Key, typename Value>
bool QxCollection<Key, Value>::replace(long index, const Key &key,
                                       const Value &value) {
  qAssert(!exist(key));
  QMutexLocker locker(&m_mutex);
...
template <typename Key, typename Value>
typename QxCollection<Key, Value>::const_reference_value
QxCollection<Key, Value>::getByKey(const Key &key) const {
  qAssert(exist(key));
  QMutexLocker locker(&m_mutex);
  qAssert(m_list.size() == m_hash.size());
...
template <typename Key, typename Value>
typename QxCollection<Key, Value>::const_reference_value
QxCollection<Key, Value>::getByIndex(long index) const {
  QMutexLocker locker(&m_mutex);
  qAssert(m_list.size() == m_hash.size());
...
template <typename Key, typename Value>
typename QxCollection<Key, Value>::const_reference_value
QxCollection<Key, Value>::getFirst() const {
  qAssert(!empty());
  QMutexLocker locker(&m_mutex);
  qAssert(m_list.size() == m_hash.size());
...
template <typename Key, typename Value>
typename QxCollection<Key, Value>::const_reference_value
QxCollection<Key, Value>::getLast() const {
  qAssert(!empty());
  QMutexLocker locker(&m_mutex);
  qAssert(m_list.size() == m_hash.size());
...
template <typename Key, typename Value>
typename QxCollection<Key, Value>::const_reference_key
QxCollection<Key, Value>::getKeyByIndex(long index) const {
  QMutexLocker locker(&m_mutex);
  qAssert(m_list.size() == m_hash.size());
  qAssert((index >= 0) && (index < static_cast<long>(m_list.size())));
  qAssert(m_hash.value(m_list.at(index).first, -1) == index);
...
void QxCollection<Key, Value>::sortByKey(bool bAscending /* = true */) {
  if (bAscending) {
    ...
    std::sort(m_list.begin(), m_list.end(),
              (&compareKeyValue < std::is_pointer<Key>::value ||
                   qx::trait::is_smart_ptr<Key>::value,
               0 > ::compareByKeyAscending));
  } else {
    QMutexLocker locker(&m_mutex);
    std::sort(m_list.begin(), m_list.end(),
              (&compareKeyValue < std::is_pointer<Key>::value ||
                   qx::trait::is_smart_ptr<Key>::value,
               0 > ::compareByKeyDescending));
  }
  updateHashPosition(0, -1, true);
}
...
void QxCollection<Key, Value>::sortByValue(bool bAscending /* = true */) {
  if (bAscending) {
    QMutexLocker locker(&m_mutex);
    std::sort(m_list.begin(), m_list.end(),
              (&compareKeyValue < std::is_pointer<Value>::value ||
                   qx::trait::is_smart_ptr<Value>::value,
               0 > ::compareByValueAscending));
  } else {
    QMutexLocker locker(&m_mutex);
    std::sort(m_list.begin(), m_list.end(),
              (&compareKeyValue < std::is_pointer<Value>::value ||
                   qx::trait::is_smart_ptr<Value>::value,
               0 > ::compareByValueDescending));
  }
  updateHashPosition(0, -1, true);
}
