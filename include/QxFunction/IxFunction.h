 ...
 * \author Lionel Marty
 * \ingroup QxFunction
 * \brief Common interface for all functions registered into QxOrm context (used
 * by introspection engine)
 */
...
/*!
 * \ingroup QxFunction
 * \brief qx::IxFunction : common interface for all functions registered into
 * QxOrm context (used by introspection engine)
 */
...
protected:
  ...
  QString
      m_sSeparator; //!< Separator character(s) for 'QString' parameters type
...
public:
  ...
  virtual qx_bool invoke(const QString &params = QString(),
                         qx::any *ret = NULL) const = 0;
  virtual qx_bool invoke(const type_any_params &params,
                         qx::any *ret = NULL) const = 0;
  virtual qx_bool invoke(void *pOwner, const QString &params = QString(),
                         qx::any *ret = NULL) const = 0;
  virtual qx_bool invoke(void *pOwner, const type_any_params &params,
                         qx::any *ret = NULL) const = 0;
  ...
  template <class T> qx_bool isValidOwner(void *pOwner, T *dummy) const {
    Q_UNUSED(dummy);
  ...
  template <class T> qx_bool isValid(const T &params) const {
