 ...
 * \author Lionel Marty
 * \ingroup QxDao
 * \brief Manage a relationship one-to-one defined between 2 classes (or between
...
/*!
 * \ingroup QxDao
 * \brief qx::QxSqlRelation_OneToOne<DataType, Owner> : manage a relationship
 * one-to-one defined between 2 classes (or between 2 tables in database)
 */
...
public:
  ...
  virtual QString getDescription() const {
    return QStringLiteral("relation one-to-one");
  }
  ...
  virtual QSqlError onAfterSave(QxSqlRelationParams &params) const {
    ...
    if (!params.recursiveMode()) {
      ...
      return qx::dao::save_with_relation_recursive(
          this->getData(params), params.saveMode(), (&params.database()),
          (&params));
    }
  }
  ...
  virtual void *eagerFetch_ResolveOutput(QxSqlRelationParams &params) const {
    ...
    while ((p = this->nextData(lIndex))) {
      if (params.checkColumns(p->getKey())) {
        p->fromVariant((&currData), query.value(lCurrIndex + lOffsetRelation),
