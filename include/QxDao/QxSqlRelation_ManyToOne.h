 ...
 * \author Lionel Marty
 * \ingroup QxDao
 * \brief Manage a relationship many-to-one defined between 2 classes (or
...
/*!
 * \ingroup QxDao
 * \brief qx::QxSqlRelation_ManyToOne<DataType, Owner> : manage a relationship
 * many-to-one defined between 2 classes (or between 2 tables in database)
 */
...
public:
  QxSqlRelation_ManyToOne(IxDataMember *p) : QxSqlRelation<DataType, Owner>(p) {
    this->setRelationType(qx::IxSqlRelation::many_to_one);
  }
  virtual ~QxSqlRelation_ManyToOne() { ; }

  virtual QString getDescription() const {
    return QStringLiteral("relation many-to-one");
  }
  ...
  virtual void lazyFetch_ResolveOutput(QxSqlRelationParams &params) const {
    ...
    typename QxSqlRelation<DataType, Owner>::type_owner &currOwner =
    ...
    if (pData && bValidId) {
      for (int i = 0; i < pData->getNameCount(); i++) {
        pData->fromVariant((&currOwner), query.value(params.offset() + i), i,
  ...
  virtual void *eagerFetch_ResolveOutput(QxSqlRelationParams &params) const {
    ...
    long lOffsetData = (pData ? pData->getNameCount() : 0);
    ...
    if (pData && bValidId) {
      for (int i = 0; i < pData->getNameCount(); i++) {
        pData->fromVariant((&currOwner), query.value(lOffsetOld + i), i,
                           qx::cvt::context::e_database);
      }
    }
    if (!bValidIdBis) {
    ...
    if (pId) {
      for (int i = 0; i < pId->getNameCount(); i++) {
        pId->fromVariant((&currData), query.value(lOffsetOld + lOffsetData + i),
                         i, qx::cvt::context::e_database);
      }
    }
    while ((p = this->nextData(lIndex))) {
      if (params.checkColumns(p->getKey())) {
        p->fromVariant((&currData), query.value(lOffsetRelation++), -1,
                       qx::cvt::context::e_database);
      }
    }
  ...
  virtual void lazyInsert_ResolveInput(QxSqlRelationParams &params) const {
    ...
    qAssert(pData);
    if (pData) {
      pData->setSqlPlaceHolder(query, (&currOwner));
    }
  }
};
