 ...
 * \author Lionel Marty
 * \ingroup QxDao
 * \brief qx::dao::ptr<T> : provide a classic smart-pointer (like
 * boost::shared_ptr<T> or QSharedPointer<T>) with some features associated with
 * QxDao module of QxOrm library
 */
 ...
 * QxOrm smart-pointer is based on QSharedPointer and provides new features with
qx::dao::xxx functions of QxDao module.
 * qx::dao::ptr<T> keeps automatically values from database.
 * So it's possible to detect if an instance has been modified using the method
isDirty() : this method can return list of properties changed.
 * qx::dao::ptr<T> can also be used with the function
qx::dao::update_optimized() to update in database only properties changed.
 * qx::dao::ptr<T> can be used with a simple object and with many containers :
...
if (bDirty) { qDebug("[QxOrm] test dirty 1 : blog is dirty => '%s'",
qPrintable(lstDiff.join("|"))); }
...
type_lst_author_test_is_dirty(new QList<author_ptr>()); daoError =
qx::dao::fetch_all(container_isdirty); qAssert(! daoError.isValid() && !
container_isdirty.isDirty() && (container_isdirty->count() == 3));

author_ptr author_ptr_dirty = container_isdirty->at(1);
author_ptr_dirty->m_name = "author name modified at index 1 => container is
dirty !!!"; bDirty = container_isdirty.isDirty(lstDiff); qAssert(bDirty &&
(lstDiff.count() == 1)); if (bDirty) { qDebug("[QxOrm] test dirty 2 : container
is dirty => '%s'", qPrintable(lstDiff.join("|"))); }
...
if (bDirty) { qDebug("[QxOrm] test dirty 3 : container is dirty => '%s'",
qPrintable(lstDiff.join("|"))); }

// Update only property 'm_name' at position 1, only property 'm_birthdate' at
position 2 and nothing at position 0 daoError =
qx::dao::update_optimized(container_isdirty); qAssert(! daoError.isValid() && !
container_isdirty.isDirty()); qx::dump(container_isdirty);
...
daoError = qx::dao::fetch_all(lst_blog_with_only_date_creation, NULL,
lstColumns); qAssert(! daoError.isValid() &&
(lst_blog_with_only_date_creation.size() > 0));

if ((lst_blog_with_only_date_creation.size() > 0) &&
(lst_blog_with_only_date_creation[0] != NULL)) {
...
private:
  QSharedPointer<T>
      m_pWork; //!< Default pointer => user works with this pointer
  QSharedPointer<T> m_pOriginal; //!< Keep original pointer containing all
...
public:
  ...
  ptr(T *ptr, T *original, Deleter deleter)
      : m_pWork(ptr, deleter), m_pOriginal(original) {
    ;
  }
  ...
  template <class X> qx::dao::ptr<T> &operator=(const qx::dao::ptr<X> &other) {
    m_pWork = qSharedPointerCast<T>(other.m_pWork);
    m_pOriginal = qSharedPointerCast<T>(other.m_pOriginal);
    return (*this);
  }
  ...
  template <class X> qx::dao::ptr<T> &operator=(const QWeakPointer<X> &other) {
    m_pWork = qSharedPointerCast<T>(other.toStrongRef());
    m_pOriginal.clear();
    return (*this);
  }
  ...
  template <class X> qx::dao::ptr<X> staticCast() const {
    return qx::dao::ptr<X>(m_pWork.template staticCast<X>(),
                           m_pOriginal.template staticCast<X>());
  }
  template <class X> qx::dao::ptr<X> dynamicCast() const {
    return qx::dao::ptr<X>(m_pWork.template dynamicCast<X>(),
                           m_pOriginal.template dynamicCast<X>());
  }
  template <class X> qx::dao::ptr<X> constCast() const {
    return qx::dao::ptr<X>(m_pWork.template constCast<X>(),
                           m_pOriginal.template constCast<X>());
  }
  ...
  bool isDirty(QStringList &lstDiff) const {
    ...
    qx::dao::detail::QxDao_IsDirty<T>::compare((*m_pWork), (*m_pOriginal),
                                               lstDiff);
...
bool operator==(const qx::dao::ptr<T> &ptr1, const qx::dao::ptr<X> &ptr2) {
  return (ptr1.toQtSharedPointer() == ptr2.toQtSharedPointer());
}
template <class T, class X>
bool operator!=(const qx::dao::ptr<T> &ptr1, const qx::dao::ptr<X> &ptr2) {
  return (ptr1.toQtSharedPointer() != ptr2.toQtSharedPointer());
...
bool operator==(const qx::dao::ptr<T> &ptr1, const X *ptr2) {
  return (ptr1.toQtSharedPointer() == ptr2);
}
template <class T, class X>
bool operator!=(const qx::dao::ptr<T> &ptr1, const X *ptr2) {
  return (ptr1.toQtSharedPointer() != ptr2);
...
bool operator==(const T *ptr1, const qx::dao::ptr<X> &ptr2) {
  return (ptr1 == ptr2.toQtSharedPointer());
}
template <class T, class X>
bool operator!=(const T *ptr1, const qx::dao::ptr<X> &ptr2) {
  return (ptr1 != ptr2.toQtSharedPointer());
