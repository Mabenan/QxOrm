 ...
 * \author Lionel Marty
 * \ingroup QxDao
 * \brief Manage a relationship one-to-many defined between 2 classes (or
...
/*!
 * \ingroup QxDao
 * \brief qx::QxSqlRelation_OneToMany<DataType, Owner> : manage a relationship
 * one-to-many defined between 2 classes (or between 2 tables in database)
 */
...
public:
  ...
  virtual QString getDescription() const {
    return QStringLiteral("relation one-to-many");
  }
  virtual QString createExtraTable() const { return QLatin1String(""); }
  ...
  virtual QSqlError onAfterSave(QxSqlRelationParams &params) const {
    ...
    if (!params.recursiveMode()) {
      ...
      return qx::dao::save_with_relation_recursive(
  ...
  virtual void *eagerFetch_ResolveOutput(QxSqlRelationParams &params) const {
    ...
    long lOffsetForeign = (pForeign ? pForeign->getNameCount() : 0);
    ...
    for (int i = 0; i < pForeign->getNameCount(); i++) {
      ...
      bValidForeign =
          (bValidForeign || qx::trait::is_valid_primary_key(vForeign));
    }
    ...
    for (int i = 0; i < pForeign->getNameCount(); i++) {
      ...
      pForeign->fromVariant((&item_val), v, "", i,
                            qx::cvt::context::e_database);
    }
    while ((p = this->nextData(lIndex))) {
      if ((p != pForeign) && (params.checkColumns(p->getKey()))) {
        p->fromVariant((&item_val), query.value(lOffsetRelation++), -1,
                       qx::cvt::context::e_database);
      }
    }
...
private:
  void forceParentIdToAllChildren(QxSqlRelationParams &params) const {
    bool bForce =
        qx::QxSqlDatabase::getSingleton()->getForceParentIdToAllChildren();
    ...
    QVector<QVariant> vIdOwner;
    for (int i = 0; i < pIdOwner->getNameCount(); i++) {
      vIdOwner.append(
          pIdOwner->toVariant(params.owner(), i, qx::cvt::context::e_database));
    }
