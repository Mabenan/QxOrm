 ...
 * \author Lionel Marty
 * \ingroup QxDao
 * \brief Manage a relationship many-to-many defined between 2 classes (or
 * between 2 tables in database)
 */
...
/*!
 * \ingroup QxDao
 * \brief qx::QxSqlRelation_ManyToMany<DataType, Owner> : manage a relationship
 * many-to-many defined between 2 classes (or between 2 tables in database)
 */
...
private:
  ...
  typedef
      typename QxSqlRelation<DataType, Owner>::type_container type_container;
  typedef typename QxSqlRelation<DataType, Owner>::type_generic_container
...
public:
  QxSqlRelation_ManyToMany(IxDataMember *p, const QString &sExtraTable,
  ...
  virtual QString getDescription() const {
    return QStringLiteral("relation many-to-many");
  }
  ...
  virtual QVariant getIdFromQuery(bool bEager,
  ...
  virtual void *eagerFetch_ResolveOutput(QxSqlRelationParams &params) const {
    ...
    qAssert(pId);
    long lIndex = 0;
    long lOffsetId = (pId ? pId->getNameCount() : 0);
    bool bValidId(false);
    ...
    while ((p = this->nextData(lIndex))) {
      if (params.checkColumns(p->getKey())) {
        p->fromVariant((&item_val), query.value(lCurrIndex + lOffsetRelation),
                       -1, qx::cvt::context::e_database);
        lCurrIndex++;
      }
    }
  ...
  virtual QSqlError onAfterSave(QxSqlRelationParams &params) const {
    ...
    if (!params.recursiveMode()) {
      daoError =
          qx::dao::save(this->getContainer(params), (&params.database()));
    } else {
      daoError = qx::dao::save_with_relation_recursive(
          this->getContainer(params), params.saveMode(), (&params.database()),
          (&params));
    }
    if (daoError.isValid()) {
      return daoError;
...
private:
  void verifyParameters() {
    qAssert(!this->getExtraTable().isEmpty() &&
            !this->getForeignKeyOwner().isEmpty() &&
            !this->getForeignKeyDataType().isEmpty() &&
            (this->getForeignKeyOwner() != this->getForeignKeyDataType()));
  }
  ...
  QSqlError insertIntoExtraTable(QxSqlRelationParams &params) const {
    ...
    QStringList lstForeignKeyDataType =
        this->getForeignKeyDataType().split("|");
    ...
    sql += pIdOwner->getSqlName(", ", this->getForeignKeyOwner()) + ", " +
           pIdData->getSqlName(", ", this->getForeignKeyDataType());
    sql += QLatin1String(") VALUES (");
    sql +=
        pIdOwner->getSqlPlaceHolder("", -1, ", ", this->getForeignKeyOwner()) +
        ", " +
        pIdData->getSqlPlaceHolder("", -1, ", ",
