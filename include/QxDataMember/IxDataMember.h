 ...
 * \brief Common interface for all class properties registered into QxOrm
 * context
 */
...
/*!
 * \ingroup QxDataMember
 * \brief qx::IxDataMember : common interface for all class properties
...
public:
  ...
  bool isThereRelationPartOfPrimaryKey(int iIndexNamePK,
                                       IxSqlRelation *&pRelation,
                                       int &iIndexNameFK) const;
  bool isPartOfPrimaryKey(int iIndexNameFK, IxDataMember *&pPrimaryKey,
                          int &iIndexNamePK) const;
  void setRelationPartOfPrimaryKey(int iIndexNamePK, IxSqlRelation *pRelation,
                                   int iIndexNameFK);
  void setPartOfPrimaryKey(int iIndexNameFK, IxDataMember *pPrimaryKey,
                           int iIndexNamePK);

  QString getName(int iIndex, const QString &sOtherName = QString()) const;
  QString getSqlAlias(const QString &sTable = QString(),
                      bool bClauseWhere = false, int iIndexName = 0) const;
  QString getSqlType(int iIndexName = -1) const;
  QString getSqlTypeAndParams(int iIndexName = -1) const;
  QString getSqlPlaceHolder(const QString &sAppend = QString(),
                            int iIndexName = 0,
                            const QString &sSep = QStringLiteral(", "),
                            const QString &sOtherName = QString(),
                            bool bCheckFKPartOfPK = false) const;
  void setSqlPlaceHolder(QSqlQuery &query, void *pOwner,
                         const QString &sAppend = QString(),
                         const QString &sOtherName = QString(),
                         bool bCheckFKPartOfPK = false) const;
  QString
  getSqlAliasEqualToPlaceHolder(const QString &sTable = QString(),
                                bool bClauseWhere = false,
                                const QString &sAppend = QString(),
                                const QString &sSep = QStringLiteral(" AND "),
                                bool bCheckFKPartOfPK = false) const;
  QString
  getSqlNameEqualToPlaceHolder(const QString &sAppend = QString(),
                               const QString &sSep = QStringLiteral(" AND "),
                               bool bCheckFKPartOfPK = false) const;
  QString getSqlTablePointNameAsAlias(
      const QString &sTable, const QString &sSep = QStringLiteral(", "),
      const QString &sSuffixAlias = QString(), bool bCheckFKPartOfPK = false,
      const QString &sCustomAlias = QString()) const;
  QString getSqlName(const QString &sSep = QStringLiteral(", "),
                     const QString &sOtherName = QString(),
                     bool bCheckFKPartOfPK = false) const;
  QString getSqlNameAndTypeAndParams(const QString &sSep = QStringLiteral(", "),
                                     const QString &sOtherName = QString(),
                                     bool bCheckFKPartOfPK = false) const;
  ...
  virtual QVariant toVariant(const void *pOwner, const QString &sFormat,
                             int iIndexName = -1,
                             qx::cvt::context::ctx_type ctx =
                                 qx::cvt::context::e_no_context) const = 0;
  virtual qx_bool fromVariant(
      void *pOwner, const QVariant &v, const QString &sFormat,
      int iIndexName = -1,
      qx::cvt::context::ctx_type ctx = qx::cvt::context::e_no_context) = 0;

  QVariant toVariant(
      const void *pOwner, int iIndexName = -1,
      qx::cvt::context::ctx_type ctx = qx::cvt::context::e_no_context) const;
  qx_bool
  fromVariant(void *pOwner, const QVariant &v, int iIndexName = -1,
              qx::cvt::context::ctx_type ctx = qx::cvt::context::e_no_context);

#ifndef _QX_NO_JSON
  virtual QJsonValue toJson(const void *pOwner,
                            const QString &sFormat) const = 0;
  virtual qx_bool fromJson(void *pOwner, const QJsonValue &j,
...
public:
  ...
  template <typename T> T *getValuePtr(void *pOwner, bool *bOk = NULL) {
    if (bOk) {
      (*bOk) = false;
    }
    if (!getAccessDataPointer()) {
      qDebug("[QxOrm] qx::IxDataMember::getValuePtr<T>() : '%s'",
  ...
  template <typename T> T getValue(void *pOwner, bool *bOk = NULL) {
    if (!getAccessDataPointer()) {
  ...
  template <typename T> bool setValue(void *pOwner, const T &val) {
    if (!getAccessDataPointer()) {
...
private:
  template <typename T, int dummy> struct qxCannotAccessDataPointer {
    static T getValue(IxDataMember *pData, void *pOwner, bool *bOk) {
      Q_UNUSED(pData);
      Q_UNUSED(pOwner);
      qDebug(
          "[QxOrm] qx::IxDataMember::qxCannotAccessDataPointer<T>::getValue() "
          ": '%s'",
    ...
    static bool setValue(IxDataMember *pData, void *pOwner, const T &val) {
      ...
      Q_UNUSED(val);
      qDebug(
          "[QxOrm] qx::IxDataMember::qxCannotAccessDataPointer<T>::setValue() "
          ": '%s'",
          "type T not supported");
      return false;
    }
  ...
  template <int dummy> struct qxCannotAccessDataPointer<QVariant, dummy> {
    static QVariant getValue(IxDataMember *pData, void *pOwner, bool *bOk) {
      if (bOk) {
        (*bOk) = (pData != NULL);
      };
      return (pData ? pData->toVariant(pOwner, QLatin1String("")) : QVariant());
    }
    static bool setValue(IxDataMember *pData, void *pOwner,
                         const QVariant &val) {
      return (
          pData ? pData->fromVariant(pOwner, val, QLatin1String("")).getValue()
                : false);
    }
  };

  template <int dummy> struct qxCannotAccessDataPointer<QString, dummy> {
    static QString getValue(IxDataMember *pData, void *pOwner, bool *bOk) {
      if (bOk) {
        (*bOk) = (pData != NULL);
      };
      return (pData ? pData->toVariant(pOwner, QLatin1String("")).toString()
                    : QString());
    }
    static bool setValue(IxDataMember *pData, void *pOwner,
                         const QString &val) {
      QVariant tmp(val);
      return (
          pData ? pData->fromVariant(pOwner, tmp, QLatin1String("")).getValue()
                : false);
    }
  };
...
public:
  ...
  QX_IX_DATA_MEMBER_PURE_VIRTUAL_ARCHIVE(boost::archive::polymorphic_iarchive,
                                         boost::archive::polymorphic_oarchive)
#endif // _QX_SERIALIZE_POLYMORPHIC

#if _QX_SERIALIZE_BINARY
  QX_IX_DATA_MEMBER_PURE_VIRTUAL_ARCHIVE(boost::archive::binary_iarchive,
                                         boost::archive::binary_oarchive)
#endif // _QX_SERIALIZE_BINARY

#if _QX_SERIALIZE_TEXT
  QX_IX_DATA_MEMBER_PURE_VIRTUAL_ARCHIVE(boost::archive::text_iarchive,
                                         boost::archive::text_oarchive)
#endif // _QX_SERIALIZE_TEXT

#if _QX_SERIALIZE_XML
  QX_IX_DATA_MEMBER_PURE_VIRTUAL_ARCHIVE(boost::archive::xml_iarchive,
                                         boost::archive::xml_oarchive)
#endif // _QX_SERIALIZE_XML

#if _QX_SERIALIZE_PORTABLE_BINARY
  QX_IX_DATA_MEMBER_PURE_VIRTUAL_ARCHIVE(eos::portable_iarchive,
                                         eos::portable_oarchive)
#endif // _QX_SERIALIZE_PORTABLE_BINARY

#if _QX_SERIALIZE_WIDE_BINARY
  QX_IX_DATA_MEMBER_PURE_VIRTUAL_ARCHIVE(boost::archive::binary_wiarchive,
                                         boost::archive::binary_woarchive)
#endif // _QX_SERIALIZE_WIDE_BINARY

#if _QX_SERIALIZE_WIDE_TEXT
  QX_IX_DATA_MEMBER_PURE_VIRTUAL_ARCHIVE(boost::archive::text_wiarchive,
                                         boost::archive::text_woarchive)
#endif // _QX_SERIALIZE_WIDE_TEXT

#if _QX_SERIALIZE_WIDE_XML
  QX_IX_DATA_MEMBER_PURE_VIRTUAL_ARCHIVE(boost::archive::xml_wiarchive,
