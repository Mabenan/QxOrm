 ...
 * \author Lionel Marty
 * \ingroup QxCache
 * \brief qx::cache : based on singleton pattern, provide basic thread-safe
 * cache feature to backup and restore any kind of objects (for example, object
...
public:
  ...
  bool insert(const QString &sKey, const qx::any &anyObj, long lCost = 1,
...
/*!
 * \ingroup QxCache
 * \brief Set the maximum allowed total cost of the cache to l. If the current
 * total cost is greater than l, some objects are deleted immediately
 */
...
/*!
 * \ingroup QxCache
 * \brief Return true if the cache contains an object associated with key sKey;
...
/*!
 * \ingroup QxCache
 * \brief Delete the object associated with key sKey. Return true if the object
 * was found in the cache; otherwise return false
 */
...
/*!
 * \ingroup QxCache
 * \brief Insert object t into the cache with key sKey, associated cost lCost
 * and insertion date-time dt. Any object with the same key already in the cache
 * will be removed
 */
template <typename T>
inline bool set(const QString &sKey, T &t, long lCost = 1,
                const QDateTime &dt = QDateTime()) {
  qx::any obj(t);
  return qx::cache::detail::QxCache::getSingleton()->insert(sKey, obj, lCost,
                                                            dt);
}

/*!
 * \ingroup QxCache
 * \brief Return the object of type T associated with key sKey, or return
 * default instance of T() if the key does not exist in the cache
 */
template <typename T> inline T get(const QString &sKey) {
...
/*!
 * \ingroup QxCache
 * \brief Return true if object t can be fetched with associated key sKey and
 * insertion date-time dt; otherwise return false with an error description
 */
template <typename T>
inline qx_bool get(const QString &sKey, T &t, QDateTime &dt) {
  ...
  if (!qx::cache::exist(sKey)) {
    return qx_bool(
        false, 0,
        QStringLiteral("[QxOrm] qx::cache : key doesn't exist in cache"));
  }
  qx::any obj = qx::cache::detail::QxCache::getSingleton()->at(sKey);
  ...
  try {
    ...
    Q_UNUSED(err);
    return qx_bool(
        false, 0, QStringLiteral("[QxOrm] qx::cache : bad any cast exception"));
  } catch (...) {
    return qx_bool(
        false, 0, QStringLiteral("[QxOrm] qx::cache : unknown cast exception"));
  }
}

/*!
 * \ingroup QxCache
 * \brief Return true if object t can be fetched with associated key sKey;
 * otherwise return false with an error description
 */
template <typename T> inline qx_bool get(const QString &sKey, T &t) {
  QDateTime dt;
