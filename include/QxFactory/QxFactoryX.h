 ...
 * \author Lionel Marty
 * \ingroup QxFactory
 * \brief List of all classes registered with QxOrm library factory pattern to
 * create object instance dynamically using the class name
 */
...
/*!
 * \ingroup QxFactory
 * \brief qx::QxFactoryX : list of all classes registered with QxOrm library
 * factory pattern to create object instance dynamically using the class name
 */
...
protected:
  QHash<QString, IxFactory *>
      m_mapFactoryX;       //!< Collection of all 'IxFactory' pointer
...
private:
  ...
  static inline qx::any createInstance(const QString &sKey,
                                       bool bRawPointer = false) {
    return QxFactoryX::getSingleton()->createObject(sKey, bRawPointer);
  }
  static inline void *createInstanceNudePtr(const QString &sKey) {
    return QxFactoryX::getSingleton()->createObjectNudePtr(sKey);
  }

#ifndef _QX_NO_RTTI
  static inline const std::type_info &getTypeInfo(const QString &sKey) {
...
/*!
 * \ingroup QxFactory
 * \brief Return a smart-pointer new instance of object (std::shared_ptr<T>)
 * associated by key sKey using qx::any type (for example : qx::create("drug")
 * return a new instance of smart-pointer drug class into qx::any type)
 */
inline qx::any create(const QString &sKey, bool bRawPointer /* = false */) {
...
/*!
 * \ingroup QxFactory
 * \brief Return a nude pointer (be careful with memory leak) of type T
 * associated by key sKey, or return NULL if sKey is not registered into factory
 * engine
 */
template <typename T>
...
#else  // _QX_NO_RTTI
{
  return dynamic_cast<T *>(
      static_cast<T *>(qx::QxFactoryX::createInstanceNudePtr(sKey)));
}
#endif // _QX_NO_RTTI

/*!
 * \ingroup QxFactory
 * \brief Return a void * pointer (be careful with memory leak) associated by
 * key sKey, or return NULL if sKey is not registered into factory engine
 */
inline void *create_void_ptr(const QString &sKey) {
