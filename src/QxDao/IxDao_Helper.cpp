...
bool IxDao_Helper::updateSqlRelationX(const QStringList &relation) {
  ...
  if (m_pImpl->m_pQueryBuilder) {
    m_pImpl->m_pQueryBuilder->setHashRelation(relation.join("|"));
  }
...
void IxDao_Helper::dumpRecord() const {
  ...
  for (int i = 0; i < iCount; ++i) {
    v = record.value(i);
    sDump += (v.isNull() ? QString("NULL") : v.toString()) + QString("|");
  }
...
void IxDao_Helper::addQuery(bool bResolve) {
  ...
  if (sqlToAdd.left(6).contains("WHERE ", Qt::CaseInsensitive)) {
    sqlToAdd = sqlToAdd.right(sqlToAdd.size() - 6);
    bAddSqlCondition = true;
  } else if (sqlToAdd.left(4).contains("AND ", Qt::CaseInsensitive)) {
  ...
  if (m_pImpl->m_qxQuery.isDistinct() &&
      sql.left(7).contains("SELECT ", Qt::CaseInsensitive)) {
...
QSqlError IxDao_Helper::updateError(const QString &sError) {
  QString sDriverText =
      (QX_DAO_ERR_INTERNAL + QString(" <") + m_pImpl->m_context + QString(">"));
...
void IxDao_Helper::init(QSqlDatabase *pDatabase, const QString &sContext) {
  ...
  m_pImpl->m_bMongoDB =
      (qx::QxSqlDatabase::getSingleton()->getDriverName() == "QXMONGODB");
  ...
  if (!m_pImpl->m_bMongoDB) {
    ...
    if (!m_pImpl->m_database.isValid()) {
      updateError(QX_DAO_ERR_NO_CONNECTION);
      return;
    }
    if (!m_pImpl->m_database.isOpen() && !m_pImpl->m_database.open()) {
      updateError(QX_DAO_ERR_OPEN_CONNECTION);
      return;
    }
    if (!m_pImpl->m_pQueryBuilder) {
      updateError(QX_DAO_ERR_NO_QUERY_BUILDER);
      return;
    }
