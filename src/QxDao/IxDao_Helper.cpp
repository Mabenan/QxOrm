...
#define QX_DAO_ERR_NO_CONNECTION                                               \
  "[QxOrm] error retrieving valid sql connection to database"
#define QX_DAO_ERR_OPEN_CONNECTION                                             \
  "[QxOrm] unable to open connection to database"
...
#define QX_DAO_ERR_NO_QUERY_BUILDER                                            \
  "[QxOrm] unable to construct sql query builder"
...
#define QX_DAO_ERR_INVALID_VALUES_DETECTED                                     \
  "[QxOrm] validator engine : invalid values detected"
#define QX_DAO_ERR_READ_ONLY                                                   \
  "[QxOrm] cannot execute INSERT, UPDATE or DELETE query with a read only "    \
  "entity"

#define QX_CONSTRUCT_IX_DAO_HELPER()                                           \
  m_timeTotal(0), m_timeExec(0), m_timeNext(0), m_timePrepare(0),              \
      m_timeBuildHierarchy(0), m_timeBuildCppInstance(0),                      \
      m_timeReadCppInstance(0), m_timeBuildSql(0), m_timeOpen(0),              \
      m_timeTransaction(0), m_nextCount(0), m_lDataCount(0),                   \
      m_bTransaction(false), m_bQuiet(false), m_bTraceQuery(true),             \
      m_bTraceRecord(false), m_bCartesianProduct(false),                       \
      m_bValidatorThrowable(false), m_bNeedToClearDatabaseByThread(false),     \
      m_bMongoDB(false), m_bDisplayTimerDetails(false), m_pDataMemberX(NULL),  \
      m_pDataId(NULL), m_pSqlGenerator(NULL)

#if (QT_VERSION >= 0x040800)
#define QX_DAO_TIMER_ELAPSED(timer) timer.nsecsElapsed()
#else // (QT_VERSION >= 0x040800)
#define QX_DAO_TIMER_ELAPSED(timer)                                            \
  (timer.elapsed() * 1000000) // convert milli-seconds to nano-seconds
...
struct IxDao_Helper::IxDao_HelperImpl {
    ...
    QElapsedTimer
        m_timerTotal; //!< Timer to manage total elapsed time : build SQL query +
        //!< all database operations + fetch C++ classes
    ...
    QElapsedTimer
        m_timerPrepare; //!< Timer to manage database prepare() elapsed time
    QElapsedTimer m_timerBuildHierarchy; //!< Timer to manage build relationships
        //!< hierarchy elapsed time
    QElapsedTimer m_timerBuildCppInstance; //!< Timer to manage build C++ instance
        //!< elapsed time
    QElapsedTimer m_timerReadCppInstance;  //!< Timer to manage read C++ instance
        //!< elapsed time
    QElapsedTimer
        m_timerBuildSql;       //!< Timer to manage build SQL query elapsed time
    QElapsedTimer m_timerOpen; //!< Timer to manage open database elapsed time
    QElapsedTimer m_timerTransaction; //!< Timer to manage database transaction
        //!< (commit/rollback) elapsed time
    qint64
        m_timeTotal; //!< Time (in nanoseconds) for total elapsed time : build SQL
        //!< query + all database operations + fetch C++ classes
    qint64 m_timeExec; //!< Time (in nanoseconds) to execute SQL query in database
    qint64 m_timeNext; //!< Time (in nanoseconds) of all calls of
        //!< QSqlQuery::next() method
    qint64 m_timePrepare;        //!< Time (in nanoseconds) of all calls of
        //!< QSqlQuery::prepare() method
    qint64 m_timeBuildHierarchy; //!< Time (in nanoseconds) to build relationships
        //!< hierarchy
    qint64
        m_timeBuildCppInstance;   //!< Time (in nanoseconds) to build C++ instance
    ...
    qint64 m_timeTransaction; //!< Time (in nanoseconds) for database transaction
        //!< (commit/rollback)
    ...
    QString m_context;        //!< Description of context : fetch, insert, update,
        //!< delete, etc...
    long m_lDataCount;        //!< Data member collection count
    bool m_bTransaction; //!< Transaction in progress : commit if valid, rollback
        //!< if error
    ...
    bool m_bCartesianProduct; //!< Recordset can return cartesian product => same
        //!< id in multiple records
    bool
    ...
    QStringList m_lstColumns;  //!< List of columns to execute sql query (if empty
        //!< => all columns)
    bool m_bNeedToClearDatabaseByThread; //!< Internal purpose only to clear
        //!< current database context by thread
        //!< in destructor
    bool m_bMongoDB; //!< Current database context is a MongoDB database
    QStringList m_lstItemsAsJson; //!< List of items to insert/update/delete as
        //!< JSON (used for MongoDB database)
    ...
    IxSqlGenerator *m_pSqlGenerator; //!< SQL generator to build SQL query
        //!< specific for each database
    qx::QxInvalidValueX
        m_lstInvalidValues; //!< List of invalid values using validator engine
    qx::QxSqlRelationLinked_ptr
        m_pSqlRelationLinked; //!< List of relation linked to build a hierarchy of
        //!< relationships

    IxDao_HelperImpl(qx::IxSqlQueryBuilder *pBuilder,
                     const qx::QxSqlQuery *pQuery)
        : QX_CONSTRUCT_IX_DAO_HELPER() {
        m_pQueryBuilder.reset(pBuilder);
        if (pQuery) {
            m_qxQuery = (*pQuery);
        }
    }
...
IxDao_Helper::IxDao_Helper(qx::IxSqlQueryBuilder *pBuilder,
                           const qx::QxSqlQuery *pQuery /* = NULL */)
    : m_pImpl(new IxDao_HelperImpl(pBuilder, pQuery)) {
    ;
}

IxDao_Helper::~IxDao_Helper() {
    terminate();
    if (m_pImpl->m_bNeedToClearDatabaseByThread) {
...
bool IxDao_Helper::hasFeature(QSqlDriver::DriverFeature ft) const {
    return (m_pImpl->m_database.driver()
                ? m_pImpl->m_database.driver()->hasFeature(ft)
                : false);
}
...
qx::IxDataMember *IxDao_Helper::nextData(long &l) const {
    return (m_pImpl->m_pQueryBuilder ? m_pImpl->m_pQueryBuilder->nextData(l)
                                     : NULL);
}

QString IxDao_Helper::sql() const {
    return (m_pImpl->m_pQueryBuilder ? m_pImpl->m_pQueryBuilder->getSqlQuery()
                                     : QLatin1String(""));
}
...
bool IxDao_Helper::isReadOnly() const {
    return ((m_pImpl->m_pDataMemberX && m_pImpl->m_pDataMemberX->getClass())
                ? m_pImpl->m_pDataMemberX->getClass()->isDaoReadOnly()
                : false);
}
...
bool IxDao_Helper::prepare(QString &sql) {
    ...
    if (sqlTemp != sql) {
        qDebug("[QxOrm] SQL query has been changed by SQL generator "
               "(onBeforeSqlPrepare) :\n   - before : '%s'\n   - after : '%s'",
               qPrintable(sqlTemp), qPrintable(sql));
    }
...
void IxDao_Helper::addInvalidValues(const qx::QxInvalidValueX &lst) {
    ...
    if (m_pImpl->m_lstInvalidValues.count() > 0) {
        QString sInvalidValues = QStringLiteral(QX_DAO_ERR_INVALID_VALUES_DETECTED);
        ...
        if (m_pImpl->m_bValidatorThrowable) {
            ...
            qDebug("[QxOrm] invalid values detected, throw 'qx::validator_error' "
                   "exception : '%s'",
                   qPrintable(tmp));
        }
        if (m_pImpl->m_bValidatorThrowable) {
...
QSqlError IxDao_Helper::errFailed(bool bPrepare /* = false */) {
    ...
    bool bFormatSql =
        qx::QxSqlDatabase::getSingleton()->getFormatSqlQueryBeforeLogging();
    if (bFormatSql && m_pImpl->m_pSqlGenerator) {
        m_pImpl->m_pSqlGenerator->formatSqlQuery(this, sql);
    }
    if (bPrepare) {
        qDebug(QX_DAO_ERR_PREPARE_SQL_QUERY, qPrintable(sql));
    } else {
...
QSqlError IxDao_Helper::errNoData() { return updateError(QStringLiteral(QX_DAO_ERR_NO_DATA)); }

QSqlError IxDao_Helper::errInvalidId() {
    return updateError(QStringLiteral(QX_DAO_ERR_INVALID_PRIMARY_KEY));
}

QSqlError IxDao_Helper::errInvalidRelation() {
    return updateError(QStringLiteral(QX_DAO_ERR_INVALID_SQL_RELATION));
}

QSqlError IxDao_Helper::errReadOnly() {
    return updateError(QStringLiteral(QX_DAO_ERR_READ_ONLY));
}
...
bool IxDao_Helper::updateSqlRelationX(const QStringList &relation) {
    ...
    m_pImpl->m_pSqlRelationLinked = qx::QxSqlRelationLinked::getHierarchy(
        (m_pImpl->m_pDataMemberX ? m_pImpl->m_pDataMemberX->getClass() : NULL),
        relation, bHierarchyOk, this);
    if (!bHierarchyOk) {
        m_pImpl->m_pSqlRelationLinked.reset();
    }
    if (!bHierarchyOk) {
        QString txt = bHierarchyOk.getDesc();
        qDebug("[QxOrm] %s", qPrintable(txt));
        return false;
    }
    m_pImpl->m_bCartesianProduct =
        m_pImpl->m_pSqlRelationLinked->getCartesianProduct();
    ...
    if (m_pImpl->m_bCartesianProduct) {
        m_pImpl->m_pQueryBuilder->initIdX(
            m_pImpl->m_pSqlRelationLinked->getAllRelationCount());
    }
...
void IxDao_Helper::addQuery(bool bResolve) {
    ...
    if (sqlToAdd.leftRef(6).contains(QStringLiteral("WHERE "), Qt::CaseInsensitive)) {
        sqlToAdd = sqlToAdd.right(sqlToAdd.size() - 6);
        bAddSqlCondition = true;
    } else if (sqlToAdd.leftRef(4).contains(QStringLiteral("AND "), Qt::CaseInsensitive)) {
        sqlToAdd = sqlToAdd.right(sqlToAdd.size() - 4);
        bAddSqlCondition = true;
    }
    this->builder().replaceSqlQueryAlias(sqlToAdd);
    sql += (bAddSqlCondition ? qx::IxSqlQueryBuilder::addSqlCondition(sql)
                             : QStringLiteral(" ")) +
           sqlToAdd;
    if (m_pImpl->m_qxQuery.isDistinct() &&
        sql.leftRef(7).contains(QStringLiteral("SELECT "), Qt::CaseInsensitive)) {
...
QSqlError IxDao_Helper::updateError(const QString &sError) {
    QString sDriverText =
        (QX_DAO_ERR_INTERNAL + QStringLiteral(" <") + m_pImpl->m_context + QStringLiteral(">"));
    sDriverText += (sql().isEmpty() ? (QString)QLatin1String("") : (QStringLiteral(" : ") + sql()));
...
QSqlError IxDao_Helper::updateError(const QSqlError &error) {
    ...
    if ((error.databaseText() != m_pImpl->m_error.databaseText()) &&
        (!error.databaseText().isEmpty())) {
        m_pImpl->m_error.databaseText() =
            m_pImpl->m_error.databaseText() + "\n" + error.databaseText();
    }
    if ((error.driverText() != m_pImpl->m_error.driverText()) &&
        (!error.driverText().isEmpty())) {
        m_pImpl->m_error.driverText() =
            m_pImpl->m_error.driverText() + "\n" + error.driverText();
    }
    if (((m_pImpl->m_error.type() == QSqlError::NoError) ||
         (m_pImpl->m_error.type() == QSqlError::UnknownError)) &&
        (error.type() != QSqlError::NoError)) {
        m_pImpl->m_error = QSqlError(m_pImpl->m_error.driverText(),
                                     m_pImpl->m_error.databaseText(), error.type(),
                                     m_pImpl->m_error.nativeErrorCode());
    }
    if (m_pImpl->m_error.nativeErrorCode().toInt() == -1) {
        m_pImpl->m_error.nativeErrorCode() = error.nativeErrorCode();
    }
...
void IxDao_Helper::init(QSqlDatabase *pDatabase, const QString &sContext) {
    ...
    m_pImpl->m_bTraceQuery =
        qx::QxSqlDatabase::getSingleton()->getTraceSqlQuery();
    m_pImpl->m_bTraceRecord =
        qx::QxSqlDatabase::getSingleton()->getTraceSqlRecord();
    m_pImpl->m_bMongoDB =
        (qx::QxSqlDatabase::getSingleton()->getDriverName() == QStringLiteral("QXMONGODB"));
    m_pImpl->m_bDisplayTimerDetails =
        qx::QxSqlDatabase::getSingleton()->getDisplayTimerDetails();
    qAssert(!m_pImpl->m_context.isEmpty());

#ifndef _QX_ENABLE_MONGODB
    if (m_pImpl->m_bMongoDB) {
        qAssertMsg(false, "[QxOrm] QXMONGODB driver",
                   "_QX_ENABLE_MONGODB compilation option is required to connect "
                   "to MongoDB database");
        updateError("[QxOrm] QXMONGODB driver : _QX_ENABLE_MONGODB compilation "
                    "option is required to connect to MongoDB database");
        return;
    }
    ...
    if (!m_pImpl->m_bMongoDB) {
        ...
        if (pDatabase) {
            m_pImpl->m_bNeedToClearDatabaseByThread =
                qx::QxSqlDatabase::getSingleton()->setCurrentDatabaseByThread(
                    pDatabase);
        }
        m_pImpl->m_database =
            (pDatabase ? (*pDatabase) : qx::QxSqlDatabase::getDatabase(dbError));
        if (dbError.isValid()) {
            updateError(dbError);
            return;
        }
        if (!m_pImpl->m_database.isValid()) {
            updateError(QStringLiteral(QX_DAO_ERR_NO_CONNECTION));
            return;
        }
        if (!m_pImpl->m_database.isOpen() && !m_pImpl->m_database.open()) {
            updateError(QStringLiteral(QX_DAO_ERR_OPEN_CONNECTION));
            return;
        }
        if (!m_pImpl->m_pQueryBuilder) {
            updateError(QStringLiteral(QX_DAO_ERR_NO_QUERY_BUILDER));
            return;
        }
    ...
    m_pImpl->m_pDataMemberX =
        (m_pImpl->m_pQueryBuilder ? m_pImpl->m_pQueryBuilder->getDataMemberX()
                                  : NULL);
    m_pImpl->m_lDataCount =
        (m_pImpl->m_pQueryBuilder ? m_pImpl->m_pQueryBuilder->getDataCount() : 0);
    m_pImpl->m_pDataId =
        (m_pImpl->m_pQueryBuilder ? m_pImpl->m_pQueryBuilder->getDataId() : NULL);
    m_pImpl->m_pSqlGenerator =
        qx::QxSqlDatabase::getSingleton()->getSqlGenerator();
    m_pImpl->m_bValidatorThrowable =
        qx::QxSqlDatabase::getSingleton()->getValidatorThrowable();
}
...
void IxDao_Helper::terminate() {
    if ((m_pImpl->m_lstInvalidValues.count() > 0) &&
        ...
        if (!m_pImpl->m_bQuiet) {
            qDebug("%s", QX_DAO_ERR_UNKNOWN_ERROR);
            qAssert(false);
        }
    }
...
void IxDao_Helper::IxDao_HelperImpl::displaySqlQuery() {
    ...
    QString sql =
        ((query.isEmpty() && m_pQueryBuilder) ? m_pQueryBuilder->getSqlQuery()
                                              : query);
    bool bFormatSql =
        qx::QxSqlDatabase::getSingleton()->getFormatSqlQueryBeforeLogging();
    qx::dao::detail::IxSqlGenerator *pSqlGenerator =
        qx::QxSqlDatabase::getSingleton()->getSqlGenerator();
    qint64 iTraceSqlOnlySlowQueriesDatabase = static_cast<qint64>(
        qx::QxSqlDatabase::getSingleton()->getTraceSqlOnlySlowQueriesDatabase());
    qint64 iTraceSqlOnlySlowQueriesTotal = static_cast<qint64>(
        qx::QxSqlDatabase::getSingleton()->getTraceSqlOnlySlowQueriesTotal());
    if ((iTraceSqlOnlySlowQueriesDatabase > 0) &&
        (iTraceSqlOnlySlowQueriesTotal < 0)) {
        iTraceSqlOnlySlowQueriesTotal = 999999999;
    } else if ((iTraceSqlOnlySlowQueriesTotal > 0) &&
               (iTraceSqlOnlySlowQueriesDatabase < 0)) {
    ...
    if ((m_timeTotal >= (iTraceSqlOnlySlowQueriesTotal * 1000000)) ||
        (m_timeExec >= (iTraceSqlOnlySlowQueriesDatabase *
                        1000000))) // convert milli-seconds to nano-seconds
    {
        QString log =
            "sql query (total: " +
            ((m_timeTotal == 0)
                 ? QStringLiteral("0")
                 : QString::number((static_cast<double>(m_timeTotal) / 1000000.0),
                                   'g', 3)) +
            " ms";
        log += ", db_exec: " +
               ((m_timeExec == 0)
                    ? QStringLiteral("0")
                    : QString::number((static_cast<double>(m_timeExec) / 1000000.0),
                                      'g', 3)) +
               " ms";
        if (m_bDisplayTimerDetails) {
            log += ", db_prepare: " +
                   ((m_timePrepare == 0)
                        ? QStringLiteral("0")
                        : QString::number(
                            (static_cast<double>(m_timePrepare) / 1000000.0), 'g',
                            3)) +
                   " ms";
            log +=
                ", db_next(" + QString::number(m_nextCount) + "): " +
                ((m_timeNext == 0)
                     ? QStringLiteral("0")
                     : QString::number((static_cast<double>(m_timeNext) / 1000000.0),
                                       'g', 3)) +
                " ms";
            log +=
                ", db_open: " +
                ((m_timeOpen == 0)
                     ? QStringLiteral("0")
                     : QString::number((static_cast<double>(m_timeOpen) / 1000000.0),
                                       'g', 3)) +
                " ms";
            log += ", db_transaction: " +
                   ((m_timeTransaction == 0)
                        ? QStringLiteral("0")
                        : QString::number(
                            (static_cast<double>(m_timeTransaction) / 1000000.0),
                            'g', 3)) +
                   " ms";
            log += ", build_relations: " +
                   ((m_timeBuildHierarchy == 0)
                        ? QStringLiteral("0")
                        : QString::number(
                            (static_cast<double>(m_timeBuildHierarchy) / 1000000.0),
                            'g', 3)) +
                   " ms";
            log += ", build_sql: " +
                   ((m_timeBuildSql == 0)
                        ? QStringLiteral("0")
                        : QString::number(
                            (static_cast<double>(m_timeBuildSql) / 1000000.0), 'g',
                            3)) +
                   " ms";
            log +=
                ", build_cpp: " +
                ((m_timeBuildCppInstance == 0)
                     ? QStringLiteral("0")
                     : QString::number(
                         (static_cast<double>(m_timeBuildCppInstance) / 1000000.0),
                         'g', 3)) +
                " ms";
            log +=
                ", read_cpp: " +
                ((m_timeReadCppInstance == 0)
                     ? QStringLiteral("0")
                     : QString::number(
                         (static_cast<double>(m_timeReadCppInstance) / 1000000.0),
                         'g', 3)) +
                " ms";
        }
        log += ") : " + sql;
