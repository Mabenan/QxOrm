...
void QxInvalidValueX::dump() const {
  QString s = text();
  qDebug("[QxOrm] %s", qPrintable(s));
}

QxInvalidValue QxInvalidValueX::at(long l) const {
  return (((l < 0) || (l >= m_lstInvalidValues.count()))
              ? QxInvalidValue()
              : m_lstInvalidValues.at(l));
}

QString QxInvalidValueX::text() const {
    if (m_lstInvalidValues.count() <= 0) { return QStringLiteral("There is no invalid value");
  }
  QString sResult = "List of invalid values (" +
                    QString::number(m_lstInvalidValues.count()) + ") :";
...
void QxInvalidValueX::insert(const IxValidator *pValidator) {
  ...
  sMessage.replace(QLatin1String("%NAME%"),
                   (pDataMember ? pDataMember->getName() : QString()));
  sMessage.replace(QLatin1String("%CONSTRAINT%"),
                   pValidator->getConstraint().toString());
  QVariantList lstConstraints = pValidator->getConstraints();
  for (long l = 0; l < lstConstraints.count(); l++) {
    sMessage.replace("%CONSTRAINT_" + QString::number(l) + "%",
                     lstConstraints.at(l).toString());
  }
  if (sMessage.isEmpty()) {
    qAssert(false);
    return;
  ...
  invalidValue.setPropertyName(pDataMember ? pDataMember->getName()
                                           : QString());
...
void QxInvalidValueX::insert(const QxInvalidValueX &other) {
  for (long l = 0; l < other.count(); l++) {
    ...
    sCurrPath += ((!m_sCurrentPath.isEmpty() && !sOtherPath.isEmpty())
                      ? QStringLiteral(".")
                      : QLatin1String(""));
