...
struct QxRestApi::QxRestApiImpl {
  ...
  QString m_action;    //!< Action to execute (count, fetch_by_id, fetch_all,
                       //!< fetch_by_query, insert, update, etc...)
  QString
      m_function; //!< Entity static function to call with action
                  //!< 'call_entity_function' and signature : static QJsonValue
                  //!< myEntity::myFct(const QJsonValue & request)
  ...
  QJsonValue
      m_requestJson; //!< Request which contains all parameters (as JSON format)
  QJsonValue
      m_responseJson; //!< Response after executing the request (as JSON format)
  QJsonValue m_dataJson; //!< Data used to process request (as JSON format)
  QJsonValue
      m_errorJson; //!< Error after executing the request (as JSON format)
...
QxRestApi::QxRestApi(QObject *parent /* = NULL */)
    : QObject(parent), m_pImpl(new QxRestApiImpl()) {
  ;
}
...
QString QxRestApi::getErrorDesc() const {
  return (m_pImpl->m_error.isValid() ? (m_pImpl->m_error.driverText() + "\n" +
                                        m_pImpl->m_error.databaseText())
                                     : QString());
}

int QxRestApi::getErrorCode() const {
  return (m_pImpl->m_error.isValid()
              ? m_pImpl->m_error.nativeErrorCode().toInt()
              : 0);
}
...
QString QxRestApi::processRequest(const QString &request) {
#ifdef _QX_NO_JSON
  QString msg = "QxOrm library must be built without _QX_NO_JSON compilation "
                "option to be able to call REST API module";
  m_pImpl->m_error = QSqlError(
      msg, "Cannot process request with _QX_NO_JSON compilation option",
  ...
  QJsonValue result = (m_pImpl->m_errorJson.isNull() ? m_pImpl->m_responseJson
                                                     : m_pImpl->m_errorJson);
...
QJsonValue QxRestApi::processRequest(const QJsonValue &request) {
  ...
  if (m_pImpl->m_error.isValid()) {
    m_pImpl->buildError(m_pImpl->m_error);
...
QJsonValue
QxRestApi::QxRestApiImpl::processRequestAsArray(const QJsonValue &request) {
...
bool QxRestApi::QxRestApiImpl::createInstance() {
  ...
  if (!qx::QxClassX::implementIxPersistable(m_entity)) {
    buildError(9999, "Entity '" + m_entity +
                         "' must implement qx::IxPersistable interface");
    return false;
  }
  m_instance = qx::IxPersistable_ptr(
      static_cast<qx::IxPersistable *>(qx::create_void_ptr(m_entity)));
  if (!m_instance) {
    buildError(9999, "Entity '" + m_entity +
                         "' is not valid : unable to create a new "
                         "qx::IxPersistable instance");
    return false;
  }
  ...
  // of instances (hash-map key/value)
  if (m_dataJson.isArray()) {
    std::shared_ptr<qx::IxPersistableCollection> lst;
    QJsonArray dataArray = m_dataJson.toArray();
    if (dataArray.count() <= 0) {
      buildError(9999, "Data array is empty for entity '" + m_entity + "'");
      return false;
    }
    QJsonValue dataFirst = dataArray.first();
    if (dataFirst.isNull()) {
      buildError(9999, "First item in data array is null for entity '" +
                           m_entity + "'");
      return false;
    }
    if (!dataFirst.isObject()) {
      buildError(9999,
                 "First item in data array is not an object for entity '" +
                     m_entity + "'");
      return false;
    }
    QJsonObject dataFirstObject = dataFirst.toObject();
    if (dataFirstObject.isEmpty()) {
      buildError(9999, "First object in data array is empty for entity '" +
                           m_entity + "'");
      return false;
    }
  ...
  qx_bool bFromJson =
      (m_dataJson.isNull() ? qx_bool(true) : m_instance->fromJson_(m_dataJson));
  if (!bFromJson) {
    buildError(static_cast<int>(bFromJson.getCode()), bFromJson.getDesc());
    return false;
  }
  return true;
}
...
bool QxRestApi::QxRestApiImpl::parseRequest(const QString &request) {
  ...
  if (jsonError.error != QJsonParseError::NoError) {
    buildError(static_cast<int>(jsonError.error),
               "Error parsing JSON request : " + jsonError.errorString());
    return false;
  }
...
bool QxRestApi::QxRestApiImpl::decodeRequest() {
  ...
  if (request.contains("data")) {
    ...
    if (!bParseOk) {
      buildError(static_cast<int>(bParseOk.getCode()), bParseOk.getDesc());
      return false;
    }
  }
...
bool QxRestApi::QxRestApiImpl::executeAction() {
  ...
  try {
    ...
    buildError(9999, "Unknown exception occurred executing action '" +
                         m_action + "'");
    return false;
  }
...
qx_bool QxRestApi::QxRestApiImpl::callEntityFunction() {
  ...
  if (!qx::QxClassX::getFctStatic(m_entity, m_function, true)) {
    buildError(9999,
               "Unable to call entity function : '" + m_entity +
                   "::" + m_function +
                   "' function not found (or not registered in QxOrm context)");
    return qx_bool(false);
  }
  qx_bool bInvokeFct = qx::QxClassX::invokeStatic(m_entity, m_function,
...
QJsonValue QxRestApi::QxRestApiImpl::getMetaData(IxClass *pClass) {
  ...
  if (pDataMemberX) {
    for (long l = 0; l < pDataMemberX->count(); l++) {
      IxDataMember *p = pDataMemberX->get(l);
      if (!p || (p == pDataMemberId)) {
        continue;
      }
